...
const selectedProjectId = ref<number | null>(
  projectId.value > 0 ? projectId.value : null,
);
...
const passRate = computed(() =>
  latest.value && latest.value.totalCases
    ? Math.round((latest.value.passedCases / latest.value.totalCases) * 100)
    : 0,
);
const failedResults = computed(
  () => detail.value?.results.filter((item) => item.status === "FAIL") || [],
);
...
async function refresh() {
  ...
  if (remoteRuns[0])
    detail.value = await securityApi.detail(
      workspaceId.value,
      remoteRuns[0].id,
    );
}
...
async function waitForRun(runId: number | string) {
  for (let attempt = 0; attempt < 90; attempt += 1) {
    ...
    if (next.run.status !== "RUNNING") {
      runs.value = [
        next.run,
        ...runs.value.filter((item) => String(item.id) !== String(runId)),
      ];
...
async function runHarness() {
  ...
  loading.value = true;
  try {
    const run = await securityApi.start(workspaceId.value, {
      projectId: selectedProjectId.value,
      includeModelProbes: includeModelProbes.value,
    });
...
onMounted(async () => {
  try {
    ...
    error.value =
      reason instanceof Error ? reason.message : "安全审计数据加载失败";
  }
});
</script>
...
<template>
  <Layout
    ...
    title="安全审计 Harness"
    subtitle="用可重复的应用安全用例检查提示词、工具、租户边界、模型参数、网络出口和秘密脱敏。"
  >
    <section class="settings-section security-hero">
      <div class="security-hero-copy">
        <span class="security-icon"><ShieldCheck :size="20" /></span>
        <div>
          <h2>安全基线</h2>
          <p>
            标准 Harness
            默认只执行本地确定性检查，不会调用外部模型；模型行为探针需由管理员主动开启。
          </p>
        </div>
      </div>
      <div class="security-controls">
        <label class="field-label"
          >运行时项目<select v-model="selectedProjectId">
        ...
        ><label class="probe-toggle"
          ><input
            v-model="includeModelProbes"
            type="checkbox"
          />运行模型行为探针<small>会消耗模型额度并发送测试提示词</small></label
        ><button
          ...
          @click="runHarness"
        >
          <Play :size="14" />{{ loading ? "审计运行中" : "运行安全审计" }}
        </button>
      </div>
    ...
    <section class="security-summary">
      <div class="summary-card">
        <strong>{{ latest?.score ?? 0 }}%</strong><span>安全得分</span>
      </div>
      <div class="summary-card">
        <strong>{{ passRate }}%</strong><span>通过率</span>
      </div>
      <div class="summary-card">
        <strong>{{ latest?.failedCases ?? 0 }}</strong
        ><span>失败用例</span>
      </div>
      <div class="summary-card">
        <strong>{{ latest ? formatTime(latest.createdAt) : "暂无" }}</strong
        ><span>最近运行</span>
      </div>
    </section>
    <section class="settings-section">
      <div class="section-intro">
        <div>
          <h2>Harness 用例</h2>
          <p>失败项会保留最小化证据；敏感凭证不会进入结果或审计详情。</p>
        </div>
      </div>
      <div class="harness-list">
        <div v-for="item in cases" :key="item.code" class="harness-row">
          <span class="case-status"
            ><component
              :is="
                resultIcon(
                  detail?.results.find(
                    (result) => result.caseCode === item.code,
                  )?.status || 'SKIPPED',
                )
              "
              :size="15"
          ...
          <span
            ...
            :class="
              resultClass(
                detail?.results.find((result) => result.caseCode === item.code)
                  ?.status || '未运行',
              )
            "
            >{{
      ...
      <div v-if="failedResults.length" class="failure-list">
        <strong>需要处理</strong>
        <p v-for="item in failedResults" :key="item.caseCode">
    ...
    <section class="settings-section split-section">
      <div>
        ...
        <div class="compact-list">
          <div v-for="run in runs" :key="run.id" class="compact-row">
            <span>#{{ run.id }}</span
            ><strong>{{ run.status }}</strong
            ><small
              >{{ run.passedCases }}/{{ run.totalCases }} 通过 ·
              {{ formatTime(run.createdAt) }}</small
            >
          </div>
      ...
      <div>
        ...
        <div class="compact-list">
          ...
          <p v-if="!auditLogs.length" class="muted">暂无审计事件</p>
        </div>
      </div>
    </section>
  </Layout>
