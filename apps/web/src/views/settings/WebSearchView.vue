...
const configured = computed(() =>
  Boolean(
    config.value?.provider === form.provider &&
    (config.value.hasCredential || !requiresApiKey.value),
  ),
);
...
async function load() {
  ...
  try {
    ...
    if (resolvedProjectId > 0)
      loadForm(
        await settingsApi.webSearch.get(workspaceId.value, resolvedProjectId),
      );
  } catch (cause) {
...
async function save() {
  ...
  try {
    loadForm(
      await settingsApi.webSearch.save(
        workspaceId.value,
...
async function testConnection() {
  ...
  error.value = "";
  try {
    const result = await settingsApi.webSearch.test(
      workspaceId.value,
      settingsProjectId.value,
    );
  ...
  >
    <section class="settings-section web-search-section">
      <div class="web-search-heading">
        <div>
          <h2>搜索提供商</h2>
          <p>
            聊天中的“允许本次联网搜索”只会使用这里的配置。搜索结果进入运行记录，由你决定是否保存到知识库。
          </p>
        </div>
        <span
          class="status-badge"
          :class="
            configured && form.enabled ? 'status-indexed' : 'status-muted'
          "
          ><i />{{ statusLabel }}</span
        >
      </div>

      <div v-if="loading" class="web-search-loading">
        <RefreshCw :size="16" class="spin" />正在读取联网搜索配置…
      </div>
      <template v-else>
        <p v-if="!hasProject" class="web-search-info">
          当前工作区还没有项目；联网搜索配置按项目保存，请先创建项目。
        </p>
        <fieldset class="web-search-form-fields" :disabled="!hasProject">
          <div class="web-search-card">
            <div class="web-search-card-title">
              <span class="web-search-icon"><Globe2 :size="18" /></span>
              <div>
                <strong>联网搜索适配器</strong
                ><small>{{
                  form.provider === "brave"
                    ? "Brave Search API"
                    : form.provider === "multi"
            ...
            <div class="web-search-fields">
              <label class="field-label"
                ...
                ><select v-model="form.provider" @change="changeProvider">
                  <option value="multi">多源混合搜索（推荐）</option>
                  <option value="duckduckgo">DuckDuckGo（无需 API Key）</option>
                  <option value="brave">Brave Search（需要 API Key）</option>
                </select></label
              >
              <label class="field-label"
                ><span>搜索地址</span
              ...
              <label class="field-label"
                ...
                ><input
                  ...
                  placeholder="zh-hans"
              /></label>
              <label v-if="requiresApiKey" class="field-label field-wide"
                ><span
                  >API Key
                  <small>{{
                    configured ? "已保存；留空表示保持不变" : "必填"
                  }}</small></span
                ><input
                  v-model="form.apiKey"
                  type="password"
            ...
            <label class="web-search-toggle"
              ...
              ><span
                ><strong>允许聊天使用联网搜索</strong
                ><small
                  >关闭后，聊天仍可使用知识库和历史记录，但不会发起外部搜索。</small
                ></span
              ></label
            >
          </div>
        ...
        <div class="web-search-actions">
          ...
          <button
            ...
            type="button"
            :disabled="testing || !configured || !form.enabled || !hasProject"
            @click="testConnection"
          >
            <Wifi :size="15" />{{ testing ? "测试中…" : "测试联网搜索" }}
          </button>
          <span v-if="testMessage" class="web-search-success">{{
            testMessage
          }}</span>
        </div>
    ...
    <section class="settings-section web-search-boundary">
      ...
      <div>
        <h2>边界说明</h2>
        <p>
          通用工具与连接器仍在“工具与连接器”中独立管理；它们不会被当作联网搜索提供商，也不会因为开启本开关而自动执行。
        </p>
      </div>
      <Link2 :size="17" />
    </section>
...
.web-search-card-title strong,
.web-search-card-title small {
  display: block;
}
.web-search-card-title strong {
  color: var(--workspace-text);
...
.web-search-toggle input:checked + .fake-checkbox {
  color: white;
  background: var(--teal-dark);
  border-color: var(--teal-dark);
}
.web-search-toggle strong,
.web-search-toggle small {
  display: block;
}
.web-search-toggle strong {
  color: var(--workspace-text);
...
.web-search-boundary h2 {
  color: var(--workspace-text) !important;
  font-size: 0.82rem !important;
}
.web-search-boundary p {
  margin-bottom: 0 !important;
}
.web-search-boundary > svg {
  margin-left: auto;
  margin-top: 0.45rem;
  color: var(--workspace-muted);
}
