...
const rules = reactive<ReviewRule[]>([
  {
    key: "citation",
    title: "结论必须可引用",
    detail: "每个事实性判断都要能定位到资料、页面或外部来源。",
    severity: "阻断",
    enabled: true,
  },
  {
    key: "numbers",
    title: "数字和时间必须核验",
    detail: "金额、比例、日期、规模等数字缺少来源时不得发布。",
    severity: "阻断",
    enabled: true,
  },
  {
...
const checks = computed(() => [
  {
    ...
    detail: assets.length
      ? `${indexedCount.value}/${assets.length} 份项目资料已完成索引`
      : "没有资料可供核验",
  ...
  },
  {
    ...
    detail: actionItems.length
      ? `${actionItems.length} 个行动项已登记，仍需负责人签署`
      : "未登记负责人和后续行动",
    status: "pending",
    icon: Users,
  },
...
async function loadGovernance() {
  ...
  try {
    ...
    rules.find((rule) => rule.key === "citation")!.enabled =
      policy.requireCitations;
    rules.find((rule) => rule.key === "numbers")!.enabled =
      policy.verifyNumbers;
    rules.find((rule) => rule.key === "conflict")!.enabled =
      policy.escalateConflicts;
    rules.find((rule) => rule.key === "external")!.enabled =
      policy.labelExternal;
...
<template>
  ...
  <section class="review-hero panel">
    ...
    <div class="review-hero-copy">
      <strong>没有证据，就没有结论</strong>
      <p>
        审查中心把“事实、推断、建议”分开处理。引用缺失、数字无法核验或来源互相冲突时，计划书保持在草稿状态。
      </p>
      <div class="review-hero-tags">
        <span><Ban :size="12" />发布阻断开启</span
        ><span
          ><LockKeyhole :size="12" />{{
            governanceLoaded
              ? `${enabledRuleCount} 项规则生效`
              : "规则状态不可用"
          }}</span
        ><span v-if="reviewRun"><Check :size="12" />本次检查已运行</span>
      </div>
    </div>
    <div class="review-hero-status">
      <span>当前状态</span><strong>{{ currentReviewStatus }}</strong
  ...
  <div class="review-metrics">
    <article class="review-metric">
      <span class="review-metric-icon red"><Ban :size="16" /></span>
      <div>
        <strong>{{ reviewRun ? reviewRun.blockedCount : "—" }}</strong
        ><small>最近一次阻断项</small>
      </div>
    </article>
    <article class="review-metric">
      <span class="review-metric-icon amber"><CircleAlert :size="16" /></span>
      <div>
        <strong>{{ reviewRun ? reviewRun.reviewCount : "—" }}</strong
        ><small>最近一次待核验项</small>
      </div>
    </article>
    <article class="review-metric">
      <span class="review-metric-icon teal"><Database :size="16" /></span>
      <div>
        <strong>{{ indexedCount }}/{{ assets.length || "—" }}</strong
        ><small>资料已索引</small>
      </div>
    </article>
    <article class="review-metric">
      <span class="review-metric-icon blue"><ShieldCheck :size="16" /></span>
      <div>
  ...
  <div class="review-layout">
    <section class="panel checks-panel">
      <div class="panel-heading">
        <div>
          <h2>结论发布检查</h2>
          <p>先由系统发现缺口，再由负责人核验具体断言</p>
        </div>
        <span class="review-policy-badge"
          ><ShieldCheck :size="13" />安全模式</span
        >
      </div>
      <div class="check-list">
        <div v-for="item in checks" :key="item.key" class="check-row">
          <span class="check-icon" :class="item.status"
      ...
      <div class="checks-footer">
        <span><LockKeyhole :size="13" />规则检查只报告缺口，不替代人工判断</span
        ><button
    ...
    <section class="panel rules-panel">
      <div class="panel-heading">
        ...
        <ShieldCheck :size="17" class="rules-heading-icon" />
      </div>
      <div class="rule-list">
        <label v-for="rule in rules" :key="rule.key" class="rule-row"
          ><input
            v-model="rule.enabled"
            :disabled="!governanceLoaded || reviewLoading"
            type="checkbox"
            :aria-label="rule.title"
          /><span class="rule-toggle" /><span class="rule-copy"
            ><strong>{{ rule.title }}</strong
            ><small>{{ rule.detail }}</small></span
          ><span
            class="rule-severity"
            :class="
              rule.severity === '阻断'
      ...
      <div class="rule-note">
        <ShieldAlert :size="14" /><span
          >关闭阻断规则会降低本项目可信等级，并记录在审查日志中。</span
        >
      </div>
      <button
        class="button button-secondary full-button"
        type="button"
        :disabled="savingRules || reviewLoading"
        @click="saveRules"
      >
        <Check :size="14" />{{ savingRules ? "保存中" : "保存审查策略" }}
      </button>
    </section>
  </div>
  ...
  <div class="review-lower-grid">
    <section class="panel flow-panel">
      <div class="panel-heading">
        <div>
          <h2>审查流程</h2>
          <p>按证据链顺序推进，任何阶段都可以回到资料层修正</p>
        </div>
      </div>
      <div class="review-flow">
        <div class="flow-item done">
          <span>01</span>
          <div><strong>输入登记</strong><small>目标、范围、负责人</small></div>
        ...
        <div class="flow-item">
          <span>02</span>
          <div>
            <strong>证据核验</strong><small>来源、版本、引用位置</small>
          </div>
          <Database :size="15" />
        </div>
        <div class="flow-line" />
        <div class="flow-item">
          <span>03</span>
          <div>
            <strong>反证检查</strong><small>冲突、假设、失败条件</small>
          </div>
          <CircleAlert :size="15" />
        </div>
        <div class="flow-line" />
        <div class="flow-item">
          <span>04</span>
          <div>
            <strong>人工签署</strong><small>责任人确认后才可发布</small>
          </div>
          <Users :size="15" />
        </div>
      </div>
    ...
    <section class="panel audit-panel">
      <div class="panel-heading">
        <div>
          <h2>审查记录</h2>
          <p>为每次发布保留规则版本和处理结果</p>
        </div>
        <FileText :size="17" class="rules-heading-icon" />
      </div>
      <div v-if="reviewRun" class="audit-result">
        <span class="audit-result-icon"><ShieldAlert :size="17" /></span>
        <div>
          <strong>结构化检查结果已生成</strong><small>{{ auditSummary }}</small
          ><em>{{
            formatDateTime(reviewRun.createdAt, "数据库时间不可用")
          }}</em>
        </div>
      </div>
      <div v-else class="audit-empty">
        <FileText :size="18" /><strong>暂无本次审查记录</strong
        ><span>运行结构化检查后，系统会在这里记录检查时间与阻断原因。</span>
      </div>
      <div class="audit-actions">
        <button
          class="text-button"
          type="button"
          @click="router.push(routeTo('project-evaluation'))"
        >
          打开评估用例 <ArrowRight :size="14" /></button
        ><button
          class="text-button"
          type="button"
          @click="router.push(routeTo('project-reports'))"
        >
          查看报告版本 <ArrowRight :size="14" />
        </button>
      </div>
    </section>
  </div>
...
.review-hero {
  ...
  border-color: color-mix(in oklab, var(--red) 20%, var(--workspace-border));
  background: linear-gradient(
    112deg,
    color-mix(in oklab, var(--red) 7%, var(--surface)),
    var(--surface)
  );
}
.review-hero-icon {
...
.review-hero-status {
  ...
  border-left: 0.0625rem solid var(--workspace-border);
}
.review-hero-status span,
.review-hero-status small {
  color: var(--workspace-muted);
  font-size: 0.75rem;
}
...
.review-metric-icon.red {
  background: #fff0ef;
  color: #bf5e5e;
}
.review-metric-icon.amber {
  background: #fff4df;
  color: #b37b20;
}
.review-metric-icon.teal {
  background: #e3f7f3;
  color: var(--teal-dark);
}
.review-metric-icon.blue {
  background: #e8f2fd;
  color: #4b86c8;
}
.review-layout,
...
.rule-toggle::after {
  ...
  border-radius: 50%;
  background: white;
  box-shadow: 0 0.0625rem 0.1875rem rgb(0 0 0 / 12%);
  content: "";
  transition: transform 0.18s ease;
}
.rule-row input:checked + .rule-toggle {
  background: var(--teal);
}
.rule-row input:checked + .rule-toggle::after {
