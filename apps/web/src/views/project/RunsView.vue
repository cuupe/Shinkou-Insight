...
const { router, routeTo, selectedProject, notify, workspaceId, projectId } =
  useWorkspace();
...
const filteredTasks = computed(() => {
  ...
  return tasks.value.filter((task) => {
    ...
    const matchesQuery =
      !query ||
      `${task.title} ${task.currentStep}`.toLowerCase().includes(query);
...
const counts = computed(() => ({
  ...
  failed: tasks.value.filter((task) =>
...
async function retryTask(task: QueueTask) {
  try {
    const remote = await runsApi.retry(
      workspaceId.value,
      projectId.value,
      task.runId,
    );
...
async function cancelQueuedTask(task: QueueTask) {
  try {
    const remote = await runsApi.cancel(
      workspaceId.value,
      projectId.value,
      task.runId,
    );
...
<template>
  ...
  <div class="queue-summary">
    <div class="queue-summary-card queue-summary-running">
      <span><Play :size="15" /></span><strong>{{ counts.running }}</strong
      ><small>运行中</small>
    </div>
    <div class="queue-summary-card queue-summary-queued">
      <span><Clock3 :size="15" /></span><strong>{{ counts.queued }}</strong
      ><small>等待中</small>
    </div>
    <div class="queue-summary-card queue-summary-paused">
      <span><Pause :size="15" /></span><strong>{{ counts.paused }}</strong
      ><small>已暂停</small>
    </div>
    <div class="queue-summary-card queue-summary-completed">
      <span><CheckCircle2 :size="15" /></span
      ><strong>{{ counts.completed }}</strong
      ><small>已完成</small>
    </div>
    <div class="queue-summary-card queue-summary-failed">
      <span><AlertCircle :size="15" /></span><strong>{{ counts.failed }}</strong
      ><small>失败/取消</small>
    </div>
  </div>
  ...
  <section class="panel queue-panel">
    <div class="queue-panel-heading">
      <div>
        <span class="eyebrow">TASK LIFECYCLE</span>
        <h2>执行任务</h2>
    ...
    <div class="queue-toolbar">
      <label class="queue-search"
        ><Search :size="14" /><input
          ...
          type="search"
          aria-label="搜索任务"
          placeholder="搜索任务或当前步骤" /></label
      ><select v-model="statusFilter" aria-label="筛选任务状态">
      ...
      ><select v-model="priorityFilter" aria-label="筛选任务优先级">
    ...
    <div class="queue-table-wrap">
      <div class="queue-table queue-table-header">
        <span>任务</span><span>状态</span><span>进度</span><span>优先级</span
        ><span>更新时间</span><span>操作</span>
      </div>
      <div
        v-for="task in filteredTasks"
        :key="task.id"
        class="queue-table queue-table-row"
      >
        <button class="queue-task-cell" type="button" @click="openTask(task)">
          <span class="queue-task-icon" :class="statusClass(task.status)"
            ><Play v-if="task.status === 'running'" :size="13" /><Pause
              v-else-if="task.status === 'paused'"
              :size="13" /><CheckCircle2
              v-else-if="task.status === 'completed'"
              :size="13" /><AlertCircle v-else :size="13" /></span
          ><span
            ><strong>{{ task.title }}</strong
            ><small
              >调研任务 · {{ task.projectName }} · #{{ task.id }}</small
            ></span
          >
        </button>
        <span class="queue-status" :class="statusClass(task.status)"
          ><i />{{ queueStatusLabel(task.status) }}</span
        >
        <div class="queue-progress-cell">
          <div class="queue-progress-top">
        ...
        <span class="queue-priority" :class="`priority-${task.priority}`">{{
          priorityLabel(task.priority)
        }}</span>
        <span class="queue-time"
          >{{ task.time
          }}<small>{{ task.duration }} · {{ task.tokens }} tokens</small></span
        >
        <div class="queue-actions">
          ...
          ><button
            ...
            class="icon-button small queue-cancel-button"
            type="button"
            aria-label="取消任务"
            @click.stop="cancelQueuedTask(task)"
          >
      ...
      <div v-if="!filteredTasks.length" class="queue-empty">
        <Clock3 :size="18" /><strong>没有匹配的任务</strong
        ><span>请更换搜索或筛选条件。</span>
      </div>
    </div>
  </section>

  <section class="queue-flow-card">
    <div>
      <span class="queue-flow-icon"><Clock3 :size="16" /></span>
      <div>
        <strong>Agent 任务如何推进</strong>
        <p>
          队列负责调度任务，右侧执行进度负责展示单个任务的实时事件；行动项则负责记录任务完成后的人工跟进。
        </p>
      </div>
    </div>
    <div class="queue-flow-steps">
      <span>等待</span><b>→</b><span>规划</span><b>→</b><span>检索</span><b>→</b
      ><span>核对证据</span><b>→</b><span>生成回答</span><b>→</b
      ><span>完成</span>
    </div>
  </section>
</template>
...
.queue-summary-running > span {
  color: #0c887a;
  background: #e1f7f3;
}
.queue-summary-queued > span {
  color: #6d7b85;
  background: #eef2f3;
}
.queue-summary-paused > span {
  color: #b7771d;
  background: #fff2dc;
}
.queue-summary-completed > span {
  color: #2f8b5e;
  background: #e7f7ee;
}
.queue-summary-failed > span {
  color: #b65b5b;
  background: #fff0f0;
}
.queue-panel {
...
.queue-panel-heading h2 {
  margin: 0;
  color: var(--workspace-text);
  font-size: 0.875rem;
}
.queue-panel-heading p {
...
.queue-live-note i {
  ...
  border-radius: 50%;
  background: var(--teal);
  box-shadow: 0 0 0 0.1875rem #dcf6f1;
}
.queue-toolbar {
  display: flex;
  align-items: center;
...
.queue-task-cell > span:last-child {
  display: grid;
  min-width: 0;
  gap: 0.25rem;
}
.queue-task-cell strong,
.queue-task-cell small {
  overflow: hidden;
...
.queue-progress-top strong {
  color: var(--teal-dark);
  font-size: 0.75rem;
}
.queue-progress-top small {
  overflow: hidden;
  color: var(--workspace-muted);
  font-size: 0.75rem;
  text-overflow: ellipsis;
...
.queue-task-icon.queue-status-running,
.queue-task-icon.queue-status-completed,
.queue-task-icon.queue-status-failed,
.queue-task-icon.queue-status-cancelled,
.queue-task-icon.queue-status-queued,
.queue-task-icon.queue-status-paused {
...
.queue-flow-card > div:first-child {
  display: flex;
  align-items: flex-start;
  gap: 0.5625rem;
}
...
.queue-status-paused {
  color: #c58a32;
  background: color-mix(in oklab, #d7962a 14%, var(--surface));
}
.queue-summary-completed > span,
.priority-low,
.queue-status-completed {
