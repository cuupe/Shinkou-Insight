...
from tools.search_quality import fallback_queries, focused_query, freshness_window, rank_web_evidence
...
class CachedWebSearch:
    ...
    def __init__(self, provider: WebSearchProvider, cache: CacheService | None, *, ttl_seconds: int, provider_key: str = "default", parser: Any | None = None) -> None:
    ...
    async def search(self, query: str, top_k: int = 5) -> list[Evidence]:
        ...
        async def load() -> list[dict[str, Any]]:
            async def retrieve(search_query: str) -> list[Evidence]:
                return [Evidence.model_validate(item) for item in await self.provider.search(search_query, top_k=min(20, max(10, limit * 3)))]

            async def verify(items: list[Evidence]) -> list[Evidence]:
                items = rank_web_evidence(query, items, 20)
                if self.client is not None:
                    items = await hydrate_web_evidence(self.client, items, query, parser=self.parser)
            ...
            if len(result) < min(limit, 2):
                ...
                attempts = await asyncio.gather(*(retrieve(value) for value in fallback_queries(query)), return_exceptions=True)
                seen_urls = {item.url for item in candidates}
                extra = [item for attempt in attempts if isinstance(attempt, list) for item in attempt if item.url not in seen_urls]
                result = rank_web_evidence(query, [*result, *await verify(extra)], limit, require_body=True)
        ...
        return rank_web_evidence(query, [Evidence.model_validate(item) for item in (value or [])], limit, require_body=True)
