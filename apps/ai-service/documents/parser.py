...
class DocumentParser:
    ...
    def parse_bytes(
        self, data: bytes, *, file_name: str, mime_type: str | None = None
    ) -> ParsedDocument:
    ...
    def _pdf(self, data: bytes, file_name: str) -> ParsedDocument:
        ...
        if len(missing_pages) > len(ocr_pages):
            warnings.append(
                f"PDF 有 {len(missing_pages)} 个页面未提取到文本，仅 OCR 前 {len(ocr_pages)} 页"
            )
        ...
        for page_number in ocr_pages:
            ...
            if not image:
                ...
                continue
            text, ocr_warning = self._ocr_image(
                image, f"{Path(file_name).stem}-page-{page_number}.png"
            )
            if text:
        ...
        if missing_pages and not documents:
            warnings.append(
                "PDF 未提取到可索引文本；如果是扫描件，请安装 Tesseract 与 Poppler"
            )
        return ParsedDocument(
            documents,
    ...
    def _docx(self, data: bytes, file_name: str) -> ParsedDocument:
        ...
        for rel in document.part.rels.values():
            ...
            try:
                ...
                continue
            text, warning = self._ocr_image(
                blob, f"{Path(file_name).stem}-{rel.rId}.png"
            )
            if text:
                paragraphs.append(f"\n[嵌入图片 OCR]\n{text}")
                if "tesseract" not in tools:
        ...
        for slide_number, slide in enumerate(presentation.slides, start=1):
            ...
            for shape in slide.shapes:
                ...
                if getattr(shape, "shape_type", None) == 13:  # MSO_SHAPE_TYPE.PICTURE
                    ...
                    text, warning = self._ocr_image(
                        shape.image.blob,
                        f"{Path(file_name).stem}-slide-{slide_number}-{image_count}.png",
                    )
                    if text:
    ...
    def _xlsx(self, data: bytes, file_name: str) -> ParsedDocument:
        ...
        return ParsedDocument(
            documents,
            tools=["openpyxl"],
            metadata={"kind": "xlsx", "sheetCount": len(workbook.worksheets)},
        )
    ...
    def _image(self, data: bytes, file_name: str) -> ParsedDocument:
        if Path(file_name).suffix.casefold() == ".svg":
            ...
            return ParsedDocument(
                ...
                tools=["svg-text"],
                metadata={"kind": "image", "ocr": False},
            )
        text, warning = self._ocr_image(data, file_name)
        warnings = [warning] if warning else []
        if not text:
            warnings.append(
                "图片未识别到文字；当前本地分析链路提供 OCR，不包含通用视觉描述"
            )
            text = f"[图片文件]\n文件名：{file_name}\n未识别到可索引文字。"
        return ParsedDocument(
            [self._document(text, file_name=file_name)],
    ...
    def _media(self, data: bytes, file_name: str, *, kind: str) -> ParsedDocument:
        ...
        if kind == "video" and self.max_video_frames:
            for frame_number, frame in enumerate(
                self._render_video_frames(data, file_name), start=1
            ):
                frame_text, frame_warning = self._ocr_image(
                    frame, f"{Path(file_name).stem}-frame-{frame_number}.png"
                )
                if frame_text:
    ...
    def _office(self, data: bytes, file_name: str) -> ParsedDocument:
        ...
        if not command:
            return ParsedDocument(
                [
                    self._document(
                        f"[Office 文件]\n文件名：{file_name}\n未安装 LibreOffice，无法解析该旧版或开放文档格式。",
                        file_name=file_name,
                    )
                ],
            ...
            parsed.metadata.update(
                {
                    "kind": "office",
                    "converted": True,
                    "originalExtension": source.suffix.casefold(),
                }
            )
            return parsed

    def _render_pdf_page(
        self, data: bytes, file_name: str, page_number: int
    ) -> bytes | None:
    ...
    def _ocr_image(self, data: bytes, file_name: str) -> tuple[str, str | None]:
        ...
        with tempfile.TemporaryDirectory(prefix="shinkou-ocr-") as directory:
            ...
            result = self._run_external(
                [command, str(input_path), "stdout", "--psm", "3", "-l", language]
            )
            if result is None or result.returncode != 0:
    ...
    def _ocr_language(self, command: str) -> str:
        requested = [
            item.strip() for item in self.ocr_languages.split("+") if item.strip()
        ]
    ...
    def _transcribe(
        self, data: bytes, file_name: str, language: str | None
    ) -> tuple[str, str | None]:
        try:
            model = _load_whisper_model(
                self.whisper_model, self.whisper_device, self.whisper_compute_type
            )
        except ImportError:
            return (
                "",
                "语音转写未安装 faster-whisper；音频/视频仍可保存，但无法提取语音内容",
            )
            ...
            try:
                ...
                for segment in segments:
                    ...
                    if not text:
                        continue
                    lines.append(
                        f"[{_timestamp(getattr(segment, 'start', 0))} - {_timestamp(getattr(segment, 'end', 0))}] {text}"
                    )
                if not lines:
    ...
    def _probe_media(self, data: bytes, file_name: str) -> dict[str, Any] | None:
        ...
        with tempfile.TemporaryDirectory(prefix="shinkou-media-") as directory:
            ...
            try:
                ...
                duration = (
                    float(format_info.get("duration"))
