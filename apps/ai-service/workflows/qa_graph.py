...
def build_qa_graph(runtime: GraphRuntime, *, checkpointer: Any | None = None):
    ...
    async def checkpoint(state: ResearchState, node: str, title: str) -> bool:
        ...
        progress = {"VALIDATE_INPUT": 5, "PLAN": 15, "RETRIEVE_INTERNAL": 35, "EVALUATE_EVIDENCE": 48, "REWRITE_QUERY": 28, "SEARCH_WEB": 55, "SYNTHESIZE_FINDINGS": 68, "WRITE_REPORT": 82, "REVIEW_REPORT": 92, "PERSIST_RESULT": 98}.get(node, 0)
        await runtime.repository.update(state["run_id"], current_node=node, current_step=title, progress=progress)
        await runtime.events.publish(state["run_id"], "node.started", {"node": node, "title": title})
        await runtime.events.publish(state["run_id"], "run.progress", {"node": node, "progress": progress, "currentStep": title})
        return True

    async def done(state: ResearchState, node: str, title: str, detail: str = "") -> None:
        await runtime.events.publish(state["run_id"], "node.completed", {"node": node, "title": title, "detail": detail})
    ...
    async def retrieve_internal(state: ResearchState) -> dict:
        ...
        for question in questions[-3:]:
            await runtime.events.publish(state["run_id"], "tool.started", {"tool": "search_knowledge", "query": question})
            ...
            found.extend(result)
            await runtime.events.publish(state["run_id"], "tool.completed", {"tool": "search_knowledge", "count": len(result)})
        unique: dict[str, Evidence] = {
            item.id: item for item in (Evidence.model_validate(raw) for raw in state.get("evidence", []))
        }
        unique.update({item.id: item for item in found})
        items = list(unique.values())[: state.get("max_evidence", 12)]
        await done(state, "RETRIEVE_INTERNAL", "Researcher 检索项目资料", f"获得 {len(items)} 条证据")
        await runtime.events.publish(state["run_id"], "evidence.added", {"count": len(items), "source": "internal", "items": [item.model_dump() for item in items]})
    ...
    async def evaluate_evidence(state: ResearchState) -> dict:
        ...
        await done(state, "EVALUATE_EVIDENCE", "Analyst 评估证据充分性", f"下一步：{action}")
    ...
    async def search_web(state: ResearchState) -> dict:
        ...
        if not state.get("allow_web_search"):
            await done(state, "SEARCH_WEB", "Researcher 检索外部来源", "调用方未授权，已跳过")
            return {}
        web_items = await runtime.web_search_for(state["run_id"]).search(state["goal"], top_k=5)
        merged = [Evidence.model_validate(item) for item in web_items]
        await done(state, "SEARCH_WEB", "Researcher 检索外部来源", f"获得 {len(merged)} 条外部证据")
        await runtime.events.publish(state["run_id"], "evidence.added", {"count": len(merged), "source": "web", "items": [item.model_dump() for item in merged]})
        unique: dict[str, Evidence] = {
            item.id: item for item in (Evidence.model_validate(raw) for raw in state.get("evidence", []))
        }
        unique.update({item.id: item for item in merged})
        return {"evidence": [item.model_dump() for item in list(unique.values())[: state.get("max_evidence", 12)]]}
    ...
    async def synthesize_findings(state: ResearchState) -> dict:
        ...
        await runtime.events.publish(state["run_id"], "finding.added", {"findingId": finding.id, "evidenceIds": finding.evidence_ids})
    ...
    async def write_report(state: ResearchState) -> dict:
        ...
        for section in draft.sections:
            section["evidence_ids"] = [item for item in section.get("evidence_ids", []) if item in valid_ids]
        if not evidence:
            draft.limitations = ["没有检索到可引用的项目证据，结论仅能作为待验证假设。", *draft.limitations]
        await done(state, "WRITE_REPORT", "Writer 撰写研究报告")
        await runtime.events.publish(state["run_id"], "report.generated", {"title": draft.title})
    ...
    def review_route(state: ResearchState) -> str:
        if state.get("review_result", {}).get("approved") or state.get("review_attempts", 0) >= 2:
    ...
    graph.add_conditional_edges("evaluate_evidence", evidence_route, {"ENOUGH": "synthesize_findings", "MORE_INTERNAL": "rewrite_query", "NEED_WEB": "search_web"})
    ...
    graph.add_conditional_edges("review_report", review_route, {"persist_result": "persist_result", "write_report": "write_report"})
