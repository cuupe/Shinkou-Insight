...
from documents.chunker import DocumentChunk
...
class InMemoryKnowledgeStore:
    ...
    async def index(self, *, workspace_id: int, project_id: int, asset_id: int | str, asset_name: str, chunks: list[DocumentChunk], embedding: EmbeddingProvider | None = None) -> int:
...
class KnowledgeIndexer:
    ...
    async def index_bytes(self, *, data: bytes, file_name: str, mime_type: str | None, workspace_id: int, project_id: int, asset_id: int | str, embedding: EmbeddingProvider | None = None) -> dict[str, Any]:
        embedding = embedding or self.embedding
        parsed = await asyncio.to_thread(self.parser.parse_bytes, data, file_name=file_name, mime_type=mime_type)
        chunks = await asyncio.to_thread(self.chunker.split, parsed.documents)
        count = await self.store.index(workspace_id=workspace_id, project_id=project_id, asset_id=asset_id, asset_name=file_name, chunks=chunks, embedding=embedding)
        graph_entities = 0
