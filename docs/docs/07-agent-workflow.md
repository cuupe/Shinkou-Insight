...
对话消息使用同一套事件总线和工具权限，但不会强制走完整调研图。运行时会根据请求选择直接回答、逐步查证、先整理步骤或回答后检查；页面对应显示“自动选择”“边查边确认”“先整理步骤”“回答后检查”等产品文案。三条路径的结构化输出、轮次上限、重复查询拦截和降级行为见 [对话处理方式升级说明](22-reasoning-workflows.md)。
